...
export default function VisualWelcomeOverlay({ theme, onGetStarted }: WelcomeProps) {
  ...
  // 🚨 AUTO-PROCEED TIMER
  // Automatically moves to the dashboard after 6 seconds (enough time for a voiceover)
  useEffect(() => {
  ...
  return (
    <div className={`w-[350px] h-[550px] min-w-[350px] min-h-[550px] flex flex-col items-center justify-between font-sans relative overflow-hidden select-none transition-colors duration-500 ${isDark ? 'bg-[#1C1C1E] text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .animate-button-progress { animation: progress-fill 6s linear forwards; }
      ...
      <div className="relative z-10 flex flex-col items-center w-full h-full pt-[60px] pb-8 px-8">
        ...
        <div className="flex flex-col items-center w-full mt-2">
          ...
          <p className={`text-[16px] font-semibold text-center leading-snug fade-in-2 px-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
            Sensa will intelligently read web pages aloud and simplify navigation for you.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 w-full mt-auto mb-auto fade-in-3">
          ...
          <div className={`flex items-center gap-4 rounded-[20px] px-4 py-4 shadow-md transition-colors ${isDark ? 'bg-[#2C2C2E] border-2 border-gray-700' : 'bg-white border-2 border-transparent'}`}>
            ...
            <div className="flex flex-col">
              <p className={`text-[15px] font-black uppercase tracking-wide leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>Smart Reader</p>
              <p className={`text-[13px] font-medium leading-snug mt-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Listen to any page with adjustable voice speeds.</p>
            </div>
          </div>
          ...
          <div className={`flex items-center gap-4 rounded-[20px] px-4 py-4 shadow-md transition-colors ${isDark ? 'bg-[#2C2C2E] border-2 border-gray-700' : 'bg-white border-2 border-transparent'}`}>
            ...
            <div className="flex flex-col">
              <p className={`text-[15px] font-black uppercase tracking-wide leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>Voice Control</p>
              <p className={`text-[13px] font-medium leading-snug mt-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Navigate effortlessly using hands-free commands.</p>
            </div>
          </div>
